...
async def run_periodic_cleanup():
    while True:
        db = SessionLocal()
        try:
            # Gọi hàm expire thay vì delete
            expired_count = crud.expire_old_slots(db)
            if expired_count > 0:
                print(f"[AUTO-JOB] Đã chuyển {expired_count} lịch trống sang trạng thái Hết Hạn.")
        except Exception as e:
            print(f"[AUTO-JOB] Lỗi: {e}")
        finally:
            db.close()
