...
// 5. [TUTOR] Xác nhận lịch hẹn (Duyệt lịch)
export const confirmBookingAPI = async (bookingId) => {
  const response = await fetch(`${API_URL}/tutor/bookings/${bookingId}/confirm`, {
    method: "POST",
    headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
  });
  ...
  if (!response.ok) {
    const err = await response.json();
    throw new Error(err.detail || "Lỗi xác nhận lịch");
  }
  return response.json();
};

// 6. [STUDENT] Yêu cầu đổi lịch
export const requestRescheduleAPI = async (bookingId, newSlotId, reason) => {
  const response = await fetch(`${API_URL}/events/request_reschedule`, {
    method: "POST",
    headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify({ 
        booking_id: bookingId, 
        new_slot_id: newSlotId, 
        reason: reason || "Sinh viên yêu cầu đổi lịch" 
    }),
  });

  if (!response.ok) {
    const err = await response.json();
    throw new Error(err.detail || "Lỗi đổi lịch");
  }
  return response.json();
};

// 10. [TUTOR] Xem danh sách lịch rảnh của chính mình
export const getTutorSelfAvailabilityAPI = async () => {
  const response = await fetch(`${API_URL}/tutor/availability`, {
    method: "GET",
    headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
  });
