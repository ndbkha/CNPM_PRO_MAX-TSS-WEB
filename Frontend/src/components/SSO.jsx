...
import { loginAPI } from "../services/api"; // Import API
...
const SSO = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // 3. Decode token để lấy Role và User Info
            // Token format: header.payload.signature
            const payload = JSON.parse(atob(token.split('.')[1]));
            localStorage.setItem("role", payload.role); // "student" hoặc "tutor"
            localStorage.setItem("email", payload.email);

            localStorage.setItem("faculty", payload.faculty); 
            ...
            console.error(err);
            setError("Tên đăng nhập hoặc mật khẩu không đúng (Kiểm tra Backend).");
        }
    };
