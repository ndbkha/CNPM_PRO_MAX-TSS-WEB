...
// Thêm prop: customOnConfirm (để xử lý đổi lịch), title (để đổi tiêu đề modal)
export default function CalendarModal({ isOpen, onClose, tutor, customOnConfirm, title }) {
  const [isModalOpen, setModalOpen] = useState(false);
  const [selectedSlot, setSelectedSlot] = useState(null);
  const [slots, setSlots] = useState([]);

  // Load slot rảnh của Tutor
  useEffect(() => {
    if (isOpen && tutor) {
        const fetchSlots = async () => {
            try {
                const data = await getTutorAvailability(tutor.id); // Dùng tutor.id lấy từ object tutor
  ...
  const handleConfirmAction = async () => {
    try {
        if (customOnConfirm) {
            // TRƯỜNG HỢP ĐỔI LỊCH: Gọi hàm được truyền từ cha xuống
            await customOnConfirm(selectedSlot.id);
        } else {
            // TRƯỜNG HỢP ĐẶT LỊCH MỚI (Mặc định)
            await createBookingAPI(tutor.id, selectedSlot.id, "Đăng ký mới");
            alert(`Đăng ký thành công Tutor: ${tutor.name} lúc ${selectedSlot.time}`);
        }
        
        setModalOpen(false); // Đóng modal xác nhận
    ...
    <>
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
        <div className="bg-white w-[600px] rounded-2xl shadow-lg p-6 relative">
          ...
          <h2 className="text-primary mb-2 flex gap-4 items-center">
            {/* Hiển thị tiêu đề tùy chỉnh hoặc mặc định */}
            <span className="text-black font-playwrite">{title || "Lịch trống của :"}</span>
            <div className="text-xl font-bold text-myred">{tutor?.name || "Tutor"}</div>
          </h2>
          <p className="text-sm text-gray-600 mb-4 ">Khoa: {tutor?.department || "..."}</p>

          <div className="space-y-3 max-h-[400px] overflow-y-auto">
            {slots.length === 0 ? <p>Tutor này chưa có lịch trống.</p> : slots.map((slot, index) => (
               <div
                  key={index}
                  className={`p-5 rounded-xl border flex justify-between items-center 
                    ${slot.available ? "bg-green-50 border-green-300" : "bg-gray-200 border-gray-300"}`}
                >
                  <div>
                    <div className="font-bold text-lg">{slot.day} - {slot.date}</div>
                    <div className="text-sm text-gray-600">
                      Tuần {slot.week} - <span className="text-primary font-bold">{slot.time}</span>
                    </div>
                  </div>

                  {slot.available ? (
                    <button
                      className="px-3 py-1 bg-primary text-white rounded-lg text-sm hover:opacity-80"
                      onClick={() => {
                        setSelectedSlot(slot);
                        setModalOpen(true);
                      }}
                    >
                      {customOnConfirm ? "Chọn đổi" : "Đăng ký"}
                    </button>
                  ) : (
                    <span className="text-red-600 font-semibold text-sm">Hết chỗ</span>
                  )}
                </div>
            ))}
          </div>
      ...
      <ConfirmModal
        ...
        title={
            customOnConfirm 
            ? `Xác nhận ĐỔI sang lịch: ${selectedSlot?.date} lúc ${selectedSlot?.time}?`
            : `Xác nhận ĐĂNG KÝ lịch: ${selectedSlot?.date} lúc ${selectedSlot?.time}?`
        }
      />
    </>
  );
